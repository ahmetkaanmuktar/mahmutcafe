...
export default function QuickAdd({ accountType, groupId, onAdded }: QuickAddProps) {
  ...
  return (
    <div className="card">
      <h2 className="font-semibold text-cafe-text mb-3">Hızlı Ekle</h2>

      <div className="flex items-center gap-3 mb-4">
        <label className="text-sm text-cafe-text">Adet:</label>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setQuantity(Math.max(1, quantity - 1))}
            className="w-8 h-8 rounded-lg bg-cafe-border text-cafe-text font-bold hover:bg-cafe-200"
          >
            −
          </button>
          <input
      ...
      <div className="grid grid-cols-2 gap-3">
        {MENU_ITEMS.filter((m) => m.id !== "ekstra").map((item) => (
          <button
            ...
            className="flex flex-col items-center gap-1 p-4 rounded-xl bg-cafe-surface hover:bg-cafe-border border border-cafe-border transition-colors disabled:opacity-50 active:scale-[0.97]"
          >
            <span className="text-2xl">{item.emoji}</span>
            <span className="font-medium text-cafe-text">{item.name}</span>
            <span className="text-sm text-cafe-textMuted">{item.price} TL</span>
          </button>
        ))}
      </div>
      ...
      {!showExtra ? (
        <button
          ...
          className="w-full mt-3 p-3 rounded-xl border-2 border-dashed border-cafe-border text-cafe-textMuted hover:border-cafe-400 hover:text-cafe-text transition-colors"
        >
          ✨ Ekstra (kendi fiyatını gir)
        </button>
      ) : (
        <div className="mt-3 p-3 rounded-xl bg-cafe-surface border border-cafe-border">
          <p className="text-sm font-medium text-cafe-text mb-2">✨ Ekstra</p>
